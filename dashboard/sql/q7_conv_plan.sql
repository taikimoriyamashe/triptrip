...
-- ソース: likes_monthly_online_revenue_forecast_inputs(VIEW)。実体は
--   sheinc_intermediate.int_likes_online_revenue_forecast_inputs を is_current_boundary AND scenario='A'
--   (=Aヨミ)で絞ったもの。spread_sheet_v2 系の Drive 外部テーブルは権限不可だが、この経路なら読める。
...
--   拠点の計画は上流 int_likes_online_revenue_forecast_inputs の kyoten_regular_conversions /
--   kyoten_stali_conversions に存在するが、このVIEWはオンライン専用タブのため露出していない。
--   現状は未取込(targets.json / UI の手入力で扱う)。取り込む場合は上流テーブルを直接参照する。
