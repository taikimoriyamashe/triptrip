...
| T3 ②③集計・派生指標 | review(全体) | scripts/aggregate.py | レビュー指摘 C1,I1〜I7,M1〜M8 を反映（主行のみ集計、リードシフト分母、分布は全個人、上位者閾値10、期間集計の社員判定、キャンセル率分母、役割粒度の統一）。v2サンプル(7/1-5)で動作確認 | コントローラー修正 → 全体レビューで再確認 |
