...
| T3 ②③集計・派生指標 | complete | scripts/aggregate.py | レビュー指摘 C1,I1〜I7,M1〜M8 を反映（主行のみ集計、リードシフト分母、分布は全個人、上位者閾値10、期間集計の社員判定、キャンセル率分母、役割粒度の統一）。v2サンプル(7/1-5)で動作確認 | コントローラー修正 → 全体レビューで再確認 |
| T4 報酬単価 | complete | README 報酬レートカード | Notion 業務規定(2026-07-16更新) | コントローラー |
| T5 定義書・納品 | complete | planner_analysis/README.md §7〜§10 数値反映済 | 全体レビュー(opus)実施中 → READY後にコミット | |
...
- 再レビュー: READY（Important/Minor 全解消、回帰なし）。
